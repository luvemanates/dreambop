...
class CartController < ApplicationController
  ...
  def show
    ...
    if current_user.nil?
      @shipping_address = ShippingAddress.where(:session_id => request.session[:session_id].to_s)
    end
    @checkout = Checkout.new(current_user, request.session[:session_id].to_s, @shipping_address)
...
protected
  def get_cart
    ...
    if request.session[:session_id].nil? || request.session[:session_id].empty?
      puts "session sesion is empty"
    end
=begin
    if request.session[:id].nil? || request.session[:id].empty?
      puts 'setting session to new hash'
...
=end
    ...
    if current_user.nil?
      @cart = Cart.where(:session_id => request.session[:session_id].to_s).first
      if @cart.nil?
        @cart = Cart.new(:user_id => 0, :session_id => request.session[:session_id])
        @cart.save
      end
    else
      @cart = current_user.cart
      if @cart.nil?
        @cart = Cart.new(:user => current_user, :session_id => request.session[:session_id])
        @cart.save
      end
    end
